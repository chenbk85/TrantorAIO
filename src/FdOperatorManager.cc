...
void FdOperatorManager::insertWriteFdop(const std::string& file_path, shared_ptr<FdOperator> fdop_ptr)
{
	std::lock_guard<mutex> lock(mtx_);
	writeFdopMap[file_path].insert(fdop_ptr);
}
void FdOperatorManager::insertReadFdop(const std::string& file_path, shared_ptr<FdOperator> fdop_ptr)
{
	std::lock_guard<mutex> lock(mtx_);
	readFdopMap[file_path].insert(fdop_ptr);
}

void FdOperatorManager::eraseWriteFdop(const std::string& file_path, shared_ptr<FdOperator> fdop_ptr)
{
	std::lock_guard<mutex> lock(mtx_);
...
void FdOperatorManager::eraseReadFdop(const std::string& file_path, shared_ptr<FdOperator> fdop_ptr)
{
	std::lock_guard<mutex> lock(mtx_);
	if(readFdopMap.find(file_path) != readFdopMap.end())
	{ 
